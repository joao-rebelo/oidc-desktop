...
public final class DesktopOAuth2AuthorizedClientExchangeFilterFunction implements ExchangeFilterFunction {
	// The following part has been copied from class org.springframework.security.oauth2.client.web.reactive.function.client.ServletOAuth2AuthorizedClientExchangeFilterFunction
	// Only the the implementation of the static method  getAuthentication(Map<String, Object>) has been changed
	...
	public DesktopOAuth2AuthorizedClientExchangeFilterFunction() {
	}

	/**
	 * Constructs a {@code DesktopOAuth2AuthorizedClientExchangeFilterFunction} using the provided parameters.
	 *
	 * <p>
	 ...
	 */
	public DesktopOAuth2AuthorizedClientExchangeFilterFunction(OAuth2AuthorizedClientManager authorizedClientManager) {
	 ...
	 *
	 * @deprecated Use {@link #DesktopOAuth2AuthorizedClientExchangeFilterFunction(OAuth2AuthorizedClientManager)} instead.
	...
	public void setClientCredentialsTokenResponseClient(
		...
		Assert.state(
				this.defaultAuthorizedClientManager,
				"The client cannot be set when the constructor used is \"DesktopOAuth2AuthorizedClientExchangeFilterFunction(OAuth2AuthorizedClientManager)\". " +
						"Instead, use the constructor \"DesktopOAuth2AuthorizedClientExchangeFilterFunction(ClientRegistrationRepository, OAuth2AuthorizedClientRepository)\".");
	 ...
	 * {@link DesktopOAuth2AuthorizedClientExchangeFilterFunction#defaultRequest()}
	 *
	...
	public void setAccessTokenExpiresSkew(Duration accessTokenExpiresSkew) {
		...
		Assert.state(
				this.defaultAuthorizedClientManager,
				"The accessTokenExpiresSkew cannot be set when the constructor used is \"DesktopOAuth2AuthorizedClientExchangeFilterFunction(OAuth2AuthorizedClientManager)\". " +
						"Instead, use the constructor \"DesktopOAuth2AuthorizedClientExchangeFilterFunction(ClientRegistrationRepository, OAuth2AuthorizedClientRepository)\".");
	 ...
	 * to construct this {@link DesktopOAuth2AuthorizedClientExchangeFilterFunction}.
	...
	private String resolveClientRegistrationId(ClientRequest request) {
		String clientRegistrationId = this.defaultClientRegistrationId;
	...
	private Mono<OAuth2AuthorizedClient> reauthorizeClient(OAuth2AuthorizedClient authorizedClient, ClientRequest request) {
		...
		OAuth2AuthorizeRequest reauthorizeRequest = OAuth2AuthorizeRequest.withAuthorizedClient(authorizedClient).principal(authentication).build();
		...
		public Mono<ClientResponse> handleResponse(ClientRequest request, Mono<ClientResponse> responseMono) {
			return responseMono
					...
					.onErrorResume(
							...
							e -> handleWebClientResponseException(request, e)
									.then(Mono.error(e)))
					.onErrorResume(
							OAuth2AuthorizationException.class,
							e -> handleAuthorizationException(request, e)
									.then(Mono.error(e)));
		}
